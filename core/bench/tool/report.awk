...
END {
    ...
    if (present < nreq) {
        ...
        print "  Suite complete. TWO candidate gate numbers exist and they are not"
        print "  interchangeable -- see WHICH BASELINE, below."
    }

    print ""
    print "  --- WHICH BASELINE (open decision, do not quote a number without it) ---"
    print ""
    print "  ROADMAP.md M3 reads: \"geometric mean overhead vs. C is <= 10%\", and"
    print "  names the quantity ARC overhead. Measured here, those are two"
    print "  different baselines, because DCDart's arithmetic TRAPS and C's does"
    print "  not:"
    print ""
    print "    vs plain C            includes trapping-arithmetic cost, which is"
    print "                          not ARC. Measured at 1.24x on fib -- a"
    print "                          benchmark with no heap and no ARC at all."
    print ""
    print "    vs trap-matched C     C built with __builtin_add_overflow /"
    print "                          __builtin_trap. Isolates ARC, as the gate's"
    print "                          WORDING specifies. fib scores 1.000x against"
    print "                          it, which is what proves the two baselines"
    print "                          differ only by the semantics DCDart chose --"
    print "                          not by flags, linkage or instrument."
    print ""
    print "  The harness reports BOTH and picks neither. Whichever is adopted, the"
    print "  other is a deliverable and not a footnote: if the trapping cost stops"
    print "  being published, choosing the matched baseline becomes indistinguishable"
    print "  from choosing a friendlier one."
    print ""
    print "  --- THE ALLOCATOR CAVEAT BELONGS NEXT TO ANY NUMBER ABOVE ---"
    print ""
    print "  The DCDart heap (ADR-0058) has NO COALESCING and NO CROSS-CLASS REUSE."
    print "  The C baseline is malloc/free, which has both. A program whose size mix"
    print "  shifts over its lifetime holds peak usage for every size class at once,"
    print "  and pays nothing here for the fragmentation malloc must handle."
    print ""
    print "  This is the single most likely thing to make an M3 number look BETTER"
    print "  than a real allocator would. It is not a reason to distrust the ratio;"
    print "  it is a reason the ratio must be published with this sentence attached."
