...
| `dcc/` | the `dcc` CLI driver | **`--target` selects the machine/OS (ADR-0033): `host`, `macos-arm64`, `linux-x86_64`, `windows-x86_64` and four more, defaulting to the original `bare-x86_64` so nothing that predates the flag changes. `--emit-header` writes a C header for FFI (ADR-0034).** **`dcc build --mode bare` produces real object files** for all twenty-eight example targets below, all passing their conformance harnesses. `--mode hosted` still throws (no backend target designed for it) |
...
Every slice was verified against the FULL conformance suite (all twenty-eight targets) after landing, zero
