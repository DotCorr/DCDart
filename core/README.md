...
| `dcc/` | the `dcc` CLI driver | **`--target` selects the machine/OS (ADR-0033): `host`, `macos-arm64`, `linux-x86_64`, `windows-x86_64` and four more, defaulting to the original `bare-x86_64` so nothing that predates the flag changes. `--emit-header` writes a C header for FFI (ADR-0034).** **`dcc build --mode bare` produces real object files** for all thirty-five example targets below, all passing their conformance harnesses. `--mode hosted` still throws (no backend target designed for it) |
...
> **36 passed, 0 failed, 0 skipped on Darwin/arm64** (35 before ADR-0057 added the `closure`
> target), but two things follow that the rows below do not say:
>
...
| `examples/m2-closure/` | M2 target (`closure.dart`, `main.c`) | **`tests/conformance/closure/run.sh`: PASS** — NON-CAPTURING closures (ADR-0057). Read the scope, not the word: a local function that captures nothing hoists to an enclosing-qualified static symbol (`twiceSum$dbl`) and is called directly; a capturing closure, or any function used as a VALUE, is a compile ERROR pointing at GAP-0052 and escalation 0008. Asserts the hoist on the SYMBOL TABLE (an implementation that inlined every local function would compute identical answers), and carries the elision baseline escalation 0008 §3 predicts will be lost: `viaTopLevel` and `viaClosure` are the same program written two ways and `dc-objdump --arc` reports byte-identical counts with the `@owned` retain/release pair genuinely elided in both |
