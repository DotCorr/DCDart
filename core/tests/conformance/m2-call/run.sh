...
# ---------------------------------------------------------------------------
# Step 3 — link main.c against calls.o, freestanding. Same Linux/x86-64-only
...
if [[ $LINK_STATUS -ne 0 ]]; then
  cat "$LINK_LOG" >&2
  fail "freestanding link of main.c + calls.o exited $LINK_STATUS (log above)"
fi
...
echo "M2-call: PASS — dcc build -> verify-freestanding pass -> freestanding link -> direct calls + call/.propagate() composition all correct"
exit 0
