...
# ---------------------------------------------------------------------------
# Step 3 — link main.c against heap_param.o, freestanding. Same Linux/
...
if [[ $LINK_STATUS -ne 0 ]]; then
  cat "$LINK_LOG" >&2
  fail "freestanding link of main.c + heap_param.o exited $LINK_STATUS (log above)"
fi
...
echo "M2-heap-param: PASS — dcc build -> verify-freestanding pass -> freestanding link -> 1000 borrowed-parameter cycles (leak-free) + 60 bounded return-ownership-transfer calls, all correct"
exit 0
